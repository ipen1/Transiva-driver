...
def main():
    ...
    headers={"Authorization":"Bearer "+a.token,"X-Device-UUID":a.device,"X-App-Scope":"driver","Cache-Control":"no-cache","User-Agent":"Transiva-Driver-Burst/1.0"}
    def once(i):
        req=urllib.request.Request(url,headers=headers,method="GET");t=time.perf_counter();code=-1;body="";err=""
        try:
            with urllib.request.urlopen(req,timeout=15) as r:code=int(r.status);body=r.read().decode("utf-8","replace")
        except urllib.error.HTTPError as e:code=int(e.code);body=e.read().decode("utf-8","replace")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=a.users) as ex:res=list(ex.map(once,range(a.requests)))
    sec=max(time.perf_counter()-started,.001);codes=Counter(str(x[0]) for x in res);times=[x[1] for x in res]
    caches=Counter(x[2] for x in res);plans=Counter(x[3] for x in res)
    ok=codes.get("200",0);rate=ok/a.requests*100;avg=statistics.mean(times);p95=pct(times,.95);p99=pct(times,.99)
    ...
    payload={"status":status,"concurrency":a.users,"requests":a.requests,"success_rate_pct":round(rate,2),"http_codes":dict(codes),
             "rps":round(a.requests/sec,2),"avg_ms":round(avg,1),"p95_ms":round(p95,1),"p99_ms":round(p99,1),
...
Endpoint yang diuji bersifat read-only: `driver_dashboard_native.php`.
"""
